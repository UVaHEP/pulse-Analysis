...
void setupScope(ps6000 &dev,   chRange &range, int samples, int nbuffers) { 
  dev.open();
...
void acquireBuffers(ps6000 &dev, vector <vector<short> > &data){
  dev.prepareBuffers();
  dev.captureBlock(); 
...
Double_t userThresholdFn(ps6000 &dev, int samples, TApplication &app) {
